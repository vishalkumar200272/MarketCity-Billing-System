...
def search():
    q2 = q.get()
    cur = mydb.cursor()
    cur.execute("SELECT * FROM products WHERE pname LIKE ?", ("%" + q2 + "%",))
    rows = cur.fetchall()
    update(rows)
...
def getTotal():
    cur = mydb.cursor()
    price = cur.execute("Select (pprice*pquantity) from products")
...
trv.bind("<ButtonRelease-1>", getrow)

style = ttk.Style()
style.configure(".", font=("Helvetica", 10, "bold"), foreground="black")  # Fixed color
...
Label(wrapper3, text="Product ID").grid(row=0, column=0, padx=5, pady=3)
Entry(wrapper3, textvariable=pid, state="disabled", width="40", font=(20)).grid(
    row=0, column=1, padx=5, pady=3, ipady=5
)

Label(wrapper3, text="Product Name").grid(row=1, column=0, padx=5, pady=3)
Entry(wrapper3, textvariable=pn, width="40", font=(20)).grid(
    row=1, column=1, padx=5, pady=3, ipady=5
)

Label(wrapper3, text="Product Category").grid(row=2, column=0, padx=5, pady=3)
...
Label(wrapper3, text="Product Quantity").grid(row=3, column=0, padx=5, pady=3)
Entry(wrapper3, textvariable=pq, width="40", font=(20)).grid(
    row=3, column=1, padx=5, pady=3, ipady=5
)

Label(wrapper3, text="Product Price").grid(row=4, column=0, padx=5, pady=3)
Entry(wrapper3, textvariable=pp, width="40", font=(20)).grid(
